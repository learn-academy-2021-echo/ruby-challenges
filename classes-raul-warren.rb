...
class Task 
    ...
    def get_info
        if @completed == false
            "The #{@title} needs/need to be done."
        else
            "The #{@title} is/are done."
...
class ColorPalette
    ...
    def all_colors
        "The colors in the color pallete are #{@color1}, #{@color2}, #{@color3}"
    end
...
# color_palette = ColorPalette.new('Blue', 'Red', 'Green')
# color_palette.color1 = 'Pink'
# color_palette.color2 = 'Purple'
# # green = ColorPalette.new('Green')
# # yellow = ColorPalette.new('Go Faster')

# p color_palette.all_colors
# # p green.color
# # p yellow.color
...
class Animal
    ...
    def get_info
        if @alive == true
            "The Animal is #{@age} years old and alive."
        else
            'The animal is no longer with us.'
        end    
    end
    def age_up
        @age = @age + 1
...
class Salmon < Fish
    ...
    super()
        ...
        def salmon_info
            if @alive == true
                "The #{@species} salmon is coldblooded, #{@age} years old, and alive."
            else
                "The #{@species} salmon is coldblooded, #{@age} years old, and dead."
            end    
        end    
        def salmon_death
                if @age >= 4
                     "The #{@species} salmon has reached the ripe old age of #{@age} and died after a full and peaceful life."
                else
                     "The #{@species} salmon is coldblooded, #{@age} years old, and alive."
                end   
        end
...
class Bear < Mammal
    ...
    def intialize()
        ...
        def bear_info 
            if @alive == true
                "The bear is #{@age} years old, warm blooded, and alive."
            else
                "The bear is dead." 
            end    
        end
            def bear_death
                if @age >= 20
                    "The bear has reached the ripe old age of #{@age} and died after a full and peaceful life."
                else
                    "The bear is #{@age} years old, warm blooded, and alive."
                end
            end
...
#  p bear.bear_death
 ...
 class Dog < Mammal
    ...
    def initialize(name,breed,sex)
        ...
        def dog_info 
            "#{@name} is a #{@sex} #{@breed} and is #{@age} years old. "
        end

 end

 cooper = Dog.new('Cooper', 'Golden Retriever', 'male')
 cooper.age = 5
 p cooper.dog_info